...
class UserSearchNewUsersModule extends UserSearchBaseModule {
    ...
    function printSearchResultsView () {
        ...
        $users = UsersModel::listNewUsers();
        $usersPerPage = parent::getUsersPerPage();
        
        ?>
        <h1><?php echo parent::getTranslation("user.search.new.title"); ?></h1>
        <?php
        if (count($users) > 0) {
            ?>
            <p><?php echo parent::getTranslation("user.search.new.description",array("%total%"=>count($users),"%amount%"=>$usersPerPage < count($users) ? $usersPerPage : count($users))); ?></p>
            <?php
            parent::listUsers($users);
        } else {
            ?>
            <p><?php echo parent::getTranslation("user.search.new.none"); ?></p>
            <?php
        }
    }
}
